...
// Helper class for exact decimal arithmetic
class DecimalNumber {
  final int value;
  final int scale;

  DecimalNumber(this.value, this.scale);

  factory DecimalNumber.fromDouble(double number) {
    number.toString();
    int scale = 1000; // We'll work with 3 decimal places
    int value = (number * scale).round();
    return DecimalNumber(value, scale);
  }
  ...
  static DecimalNumber midpoint(DecimalNumber a, DecimalNumber b) {
    assert(a.scale == b.scale);
    return DecimalNumber((a.value + b.value) ~/ 2, a.scale);
  }
}
...
class BisectionMethod {
  ...
  BisectionMethod(this.terms, {this.decimalPlaces = 3});
...
''';
  ...
  List<BisectionResult> solve({
    ...
    DecimalNumber xlDecimal = DecimalNumber.fromDouble(xl);
    DecimalNumber xuDecimal = DecimalNumber.fromDouble(xu);
    xrDecimal = DecimalNumber.midpoint(xlDecimal, xuDecimal);
    xr = xrDecimal.toDouble();
    
    debugSteps['xr_calc'] = '''
xr = (xl + xu) ÷ 2
   = (${_formatNumber(xl)} + ${_formatNumber(xu)}) ÷ 2
   = ${_formatNumber((xl + xu) / 2)}
''';

    double fxr = f(xr);
    debugSteps['f(xr)_calc'] = _getFunctionCalculationSteps(xr);

    results.add(BisectionResult(
      iteration: i,
      xl: xl,
      fxl: fxl,
      xr: xr,
...
Since f(xr) × f(xl) ≤ 0:
  ...
  f(xu_new) = f(xr) = ${_formatNumber(fxr)}''';
      ...
      // Calculate midpoint
      xlDecimal = DecimalNumber.fromDouble(xl);
      xuDecimal = DecimalNumber.fromDouble(xu);
      xrDecimal = DecimalNumber.midpoint(xlDecimal, xuDecimal);
      xr = xrDecimal.toDouble();
