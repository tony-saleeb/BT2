...
import '../models/false_position_method.dart';
import 'package:google_fonts/google_fonts.dart';
import 'package:flutter_screenutil/flutter_screenutil.dart';
import 'dart:math' show pow;

class FalsePositionSolutionScreen extends StatefulWidget {
  final List<FalsePositionResult> results;
  final String function;
  final int decimalPlaces;

  const FalsePositionSolutionScreen({
    super.key,
  ...
  @override
  State<FalsePositionSolutionScreen> createState() =>
      _FalsePositionSolutionScreenState();
}

class _FalsePositionSolutionScreenState
    extends State<FalsePositionSolutionScreen>
  ...
  void _handleTabChange() {
    // Update current tab index immediately when tab changes
    if (_currentTabIndex != _tabController.index) {
      // Use immediate setState to update UI without waiting
      setState(() {
        _currentTabIndex = _tabController.index;
      });

      // Scroll tab into view - do this outside of setState for better performance
  ...
  Widget _buildTabItem(IconData icon, String label, int index, Color color) {
    ...
    return Padding(
      ...
      child: GestureDetector(
        onTap: () {
          // Set tab controller index directly for instant switching without animation
                          ...
                          Container(
                            ...
                            child: Column(
                              ...
                              children: [
                                ...
                                Text(
                                  'The False Position Method (Regula Falsi) finds roots by using linear interpolation between two points. Unlike Bisection, it often converges faster as it considers the function values at endpoints to estimate the root location.',
                                  style: TextStyle(
                                    color: colorScheme.onSurfaceVariant,
                                    fontSize: isSmallScreen ? 14.sp : 16.sp,
                                    height: 1.6,
                                ...
                                Container(
                                  ...
                                  child: Row(
                                    children: [
                                      ...
                                      Expanded(
                                        child: Text(
                                          'Uses linear interpolation for faster convergence',
                                          style: TextStyle(
                                            color: colorScheme.secondary,
                ...
                child: DataTable(
                  ...
                  decoration: BoxDecoration(
                    ...
                    return DataRow(
                          ...
                          if (isLastRow) {
                                return colorScheme.primaryContainer.withOpacity(
                                  0.1,
                                );
                          }
                          if (states.contains(MaterialState.hovered)) {
                                return colorScheme.surfaceVariant.withOpacity(
                                  0.5,
                                );
                          }
                          return null;
                      ...
                      cells: [
                        _buildDataCell(
                          context,
                              ...
                              _buildDataCell(
                                context,
                                _formatNumber(result.xl),
                                isLastRow,
                              ),
                              _buildDataCell(
                                context,
                                _formatNumber(result.fxl),
                                isLastRow,
                              ),
                              _buildDataCell(
                                context,
                                _formatNumber(result.xr),
                                isLastRow,
                              ),
                              _buildDataCell(
                                context,
                                _formatNumber(result.fxr),
                                isLastRow,
                              ),
                              _buildDataCell(
                                context,
                                _formatNumber(result.xu),
                                isLastRow,
                              ),
                              _buildDataCell(
                                context,
                                _formatNumber(result.fxu),
                                isLastRow,
                              ),
                              _buildDataCell(
                                context,
    ...
    return DataCell(
      Container(
        width:
            ...
            text == text.contains('%') || text == '--' || text.length <= 3
                ? (isSmallScreen ? 120.w : 160.w)
                : (isSmallScreen ? 120.w : 160.w),
        decoration: BoxDecoration(
          border:
              !isLastColumn
  ...
  String _buildErrorReduction() {
    ...
    FalsePositionResult? firstWithError;
