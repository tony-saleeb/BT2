...
class BisectionMethodScreen extends StatefulWidget {
  const BisectionMethodScreen({super.key});

  @override
  State<BisectionMethodScreen> createState() => _BisectionMethodScreenState();
}

class _BisectionMethodScreenState extends State<BisectionMethodScreen> with TickerProviderStateMixin {
  ...
  void _calculateAndNavigate() {
    ...
    try {
      ...
      // First check if bounds are in correct order
      if (xl >= xu) {
      ...
      // Check if we have a defined function
      if (_terms.isEmpty) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
      ...
      // Now check bracketing condition
      final method = BisectionMethod(_terms, decimalPlaces: _decimalPlaces);
      final fxl = method.f(xl);
      final fxu = method.f(xu);
      ...
      // Calculate results using bisection method
  ...
  Widget _buildTermTile({
    ...
    if (isVariable) {
      termText += 'x';
      if (power != 1) {
        termText += _convertToSuperscript(power.toString());
      }
    }
  ...
  Future<void> _loadHistory() async {
    try {
      ...
      final historyJson = prefs.getString('bisection_history');
  ...
  Future<void> _saveHistoryToPrefs() async {
    try {
      ...
      await prefs.setString('bisection_history', json.encode(historyToSave));
    } catch (e) {
  ...
  void _saveToHistory() {
    if (_functionController.text.isNotEmpty && _xlController.text.isNotEmpty && _xuController.text.isNotEmpty) {
      ...
      // Remove any older entries with the same function and bounds
                                ...
                                const SizedBox(height: 16),
                                  ...
                                  Padding(
                                    ...
                                    child: TweenAnimationBuilder<double>(
                                      ...
                                      child: Text(
                                        'Calculations you perform will be saved here for easy access. Find roots efficiently with the Bisection Method.',
  ...
  void _validateInterval() {
    ...
    if (hasXl && hasXu) {
      try {
        ...
        // For empty function or non-mathematical validation, just check order
        if (!functionDefined) {
          setState(() {
            _isValidInterval = isProperInterval;
          });
          return;
        }
        
        // Create a temporary BisectionMethod to evaluate function values
        try {
          final tempMethod = BisectionMethod(_terms, decimalPlaces: _decimalPlaces);
          
          // Get function values at interval endpoints
          final fxl = tempMethod.f(xl);
          final fxu = tempMethod.f(xu);
          
          // For bisection method, the interval must bracket a root (opposite signs)
          final bracketsRoot = fxl * fxu <= 0;
          
          // Set validation result - for bisection method, bracketing is required
          ...
          // If function evaluation fails, just validate based on interval order
          setState(() {
            _isValidInterval = isProperInterval;
          });
          debugPrint('Function evaluation error: ${e.toString()}');
        }
      } catch (e) {
        debugPrint('Interval Validation: parsing error - ${e.toString()}');
        setState(() {
          _isValidInterval = false;
        });
      }
                    ...
                    child: Column(
                      ...
                      children: [
                        ...
                        _buildTutorialStep(
                          step: 4,
                          title: 'Calculate',
                          description: 'Hit the calculate button to find the root using the bisection method.',
                          icon: Icons.play_arrow_rounded,
                          colorScheme: colorScheme,
                        ),
                      ],
  ...
  // Function to show success animation for a button
...
// Add this new class after the BisectionMethodScreen class
class FunctionGraphPainter extends CustomPainter {
  ...
  void _drawGrid(Canvas canvas, Size size, double scale) {
    ...
    // Vertical lines
    for (int i = -5; i <= 5; i++) {
      final x = size.width / 2 + i * scale;
      canvas.drawLine(Offset(x, 0), Offset(x, size.height), paint);
    }
    
    // Horizontal lines
    for (int i = -5; i <= 5; i++) {
      final y = size.height / 2 - i * scale / 2;
  ...
  void _drawFunction(Canvas canvas, Size size, Offset center, double scale) {
    ...
    try {
      ...
      for (double pixelX = 0; pixelX < size.width; pixelX++) {
        ...
        final x = (pixelX - center.dx) / scale;
        ...
        // Convert graph y to pixel y (flipped because pixel y increases downward)
        final pixelY = center.dy - y * scale / 2;
        
        // Skip if y is too large (avoid drawing lines that go way off screen)
        if (pixelY.abs() > size.height * 2) continue;
      ...
      _drawRoots(canvas, size, center, scale);
    } catch (e) {
      debugPrint('Error drawing function: $e');
    }
  }
  
  void _drawRoots(Canvas canvas, Size size, Offset center, double scale) {
    if (terms.isEmpty) return; // Don't try to find roots if no terms exist
    ...
    try {
      double prevY = _calculateY(-5);
      
      for (double x = -4.99; x <= 5; x += 0.01) {
        final y = _calculateY(x);
        
        // Check if we crossed the x-axis
        if ((prevY < 0 && y >= 0) || (prevY > 0 && y <= 0)) {
          // Draw a point at the root (x-intercept)
          final pixelX = center.dx + x * scale;
          canvas.drawCircle(Offset(pixelX, center.dy), 6, rootPaint);
          canvas.drawCircle(Offset(pixelX, center.dy), 6, rootStrokePaint);
        }
        
        prevY = y;
      }
    } catch (e) {
      // If an error occurs, just skip drawing roots
      debugPrint('Error drawing roots: $e');
    }
  }
  
  double _calculateY(double x) {
    double y = 0;
    if (terms.isEmpty) return y;
    ...
    try {
      for (final term in terms) {
        try {
          ...
          // Skip this term if there's an error
          debugPrint('Error processing term in _calculateY: $e');
        }
      }
    } catch (e) {
      debugPrint('Error calculating y value: $e');
    }
    return y;
  }
...
class GridBackgroundPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    // Vertical lines
    for (int i = -5; i <= 5; i++) {
      if (i == 0) continue; // Skip origin line
      
      final x = center.dx + i * scale;
      final double opacity = 1.0 - (i.abs() / 6); // Fade out lines farther from center
      gridPaint.color = colorScheme.primary.withOpacity(0.05 * opacity * animationValue);
    ...
    for (int i = -10; i <= 10; i++) {
      ...
      final double opacity = 1.0 - (i.abs() / 11); // Fade out lines farther from center
      gridPaint.color = colorScheme.primary.withOpacity(0.05 * opacity * animationValue);
    ...
    // Draw subtle dots at the intersection of grid lines
    ...
    for (int i = -5; i <= 5; i++) {
      ...
      for (int j = -10; j <= 10; j++) {
        ...
        final y = center.dy - j * scale / 2;
        final double opacity = 1.0 - ((i.abs() + j.abs()) / 17);
        dotPaint.color = colorScheme.primary.withOpacity(0.05 * opacity * animationValue);
